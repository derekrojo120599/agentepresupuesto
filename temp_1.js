      ...
      // Títulos e íconos temáticos para los selectores de la aplicación
      ...
      function inicializarCustomSelect(select) {
        if (!select || select._customSelectWrapper) return;
      ...
      // Selección de opción y disparo de eventos reactivos nativos
      ...
      // Cerrar al redimensionar ventana (solo si cambia el ancho, para evitar cierres por teclado móvil)
      ...
      window.inicializarTodosLosCustomSelects = function () {
        ...
        // Inicializar Flatpickr en todos los inputs de fecha/mes para evitar escribir a mano
      ...
      function guardarCategoriasEnStorage() {
        try {
          localStorage.setItem(
            claveCategorias(),
            JSON.stringify(categoriasMap),
          );
          localStorage.setItem(
            claveIconos(),
            JSON.stringify(categoriaIconosMap),
          );
        } catch (e) {
          console.warn(e);
        }
      ...
      async function sincronizarAjustesUsuarioCloud() {
        ...
        try {
          ...
          console.warn(
            "No se pudieron sincronizar los ajustes en la nube:",
            err,
          );
        }
      }
      ...
      const contenedorDeudaSelect = document.getElementById(
        "contenedorDeudaSelect",
      );
      const deudaObjetivoSelect = document.getElementById("deudaObjetivo");
      const contenedorMetaAhorroSelect = document.getElementById(
        "contenedorMetaAhorroSelect",
      );
      ...
      document
        ...
        .addEventListener("click", crearCuenta);
      const btnGoogle = document.getElementById("btnGoogleLogin");
      ...
      supabaseClient.auth.onAuthStateChange((event, session) => {
        // Manejar posibles errores retornados en el hash de la URL tras OAuth
        ...
        if (hashParams.get("error_description")) {
          mostrarAvisoAuth(
            "Error de acceso: " +
              decodeURIComponent(hashParams.get("error_description")),
            "error",
          );
        ...
        if (session) {
          try {
            ...
            if (!sesionInicializada) {
              ...
              const onDataReady = () => {
                ...
                }, 400); // 400ms para la transición de desvanecimiento
              };
              ...
              // Si la sesión ya estaba inicializada (ej. cambio rápido de tabs), mostrar directo
              appContainer.classList.remove("hidden");
              mobileNav.classList.remove("hidden");
              mobileNav.classList.add("flex");
            }
          ...
          document.getElementById("mobileBottomNav").classList.remove("flex");
      ...
      function cambiarPestana(tabName) {
        ...
        const btnHeaderConfig = document.getElementById(
          "btnHeaderConfiguracion",
        );
        if (btnHeaderConfig) {
      ...
      function actualizarOpcionesCategoria() {
        ...
        categoriaSelect.innerHTML = categoriasMap[tipo]
          .map(
            (cat) =>
      ...
      function actualizarSelectMetas() {
        if (!metasAhorro.length) {
          metaAhorroSelect.innerHTML =
            '<option value="">Sin meta asignada (Ahorro General)</option>';
          return;
        }
        metaAhorroSelect.innerHTML =
          '<option value="">Sin meta asignada (Ahorro General)</option>' +
          metasAhorro
            .map(
              (m) =>
                `<option value="${escapeHTML(m.nombre)}">${escapeHTML(m.icono || "🎯")} ${escapeHTML(m.nombre)} (Meta: $${m.objetivo.toFixed(2)})</option>`,
            )
            .join("");
      }
      ...
      function verificarMontoEnTiempoReal() {
        ...
        // Solo aplica si es un gasto
        if (tipo === "gasto" && monto > 0) {
          // Calcular balance actual (solo Ingresos - Gastos)
          let balance = 0;
          ...
          if (monto > balance) {
            montoInput.classList.remove("border-slate-300", "dark:border-azulcielo/30", "focus:border-azulelectrico");
            ...
            montoInput.classList.add("border-slate-300", "dark:border-azulcielo/30", "focus:border-azulelectrico");
            montoInput.classList.remove("border-coral", "focus:border-coral", "text-coral");
            alertaMontoExcedido.classList.add("hidden");
      ...
      montoInput.addEventListener("input", verificarMontoEnTiempoReal);
      ...
      document
        ...
        .addEventListener("submit", agregarTransaccion);
      document
        .getElementById("formNuevaDeuda")
        .addEventListener("submit", crearDeuda);
      function poblarSelectDias() {
        ...
        selectDia.value = String(hoy.getDate()).padStart(2, "0");
      }
      poblarSelectDias();

      document
        .getElementById("formIncrementoDeuda")
        .addEventListener("submit", confirmarIncrementoDeuda);
      document
        .getElementById("formCategoria")
        .addEventListener("submit", guardarFormCategoria);
      ...
      document
        ...
        .addEventListener("change", renderizarHistorialFiltrado);
      document
        .getElementById("filtroAlcance")
      ...
      function mostrarModalAlertaInteligente(tipo, titulo, mensaje) {
        ...
        if (tipo === "exceso_presupuesto") {
          icono.className = "w-16 h-16 mx-auto rounded-full flex items-center justify-center text-3xl mb-2 bg-coral/15 text-coral shadow-[0_0_20px_rgba(244,63,94,0.3)]";
          icono.textContent = "🚨";
        } else if (tipo === "meta_superada") {
          icono.className = "w-16 h-16 mx-auto rounded-full flex items-center justify-center text-3xl mb-2 bg-emerald-500/15 text-emerald-500 shadow-[0_0_20px_rgba(16,185,129,0.3)]";
          icono.textContent = "🏆";
        } else if (tipo === "advertencia") {
          icono.className = "w-16 h-16 mx-auto rounded-full flex items-center justify-center text-3xl mb-2 bg-amber-500/15 text-amber-500 shadow-[0_0_20px_rgba(245,158,11,0.3)]";
          icono.textContent = "⚠️";
        }
      ...
      function verificarLimitesDespuesDeTransaccion(nueva) {
        ...
        if (nueva.tipo === "gasto") {
          ...
          if (limite > 0) {
            ...
            transacciones.forEach(t => {
              if (
                ...
                (typeof transaccionesPendientesEliminar === "undefined" || !transaccionesPendientesEliminar.has(t.id))
              ) {
        ...
        } else if (nueva.tipo === "ahorro" && !nueva.categoria.toLowerCase().includes("retirar")) {
          ...
          if (metaObj && metaObj.objetivo > 0) {
            ...
            if (acumulado >= metaObj.objetivo && acumuladoPrevio < metaObj.objetivo) {
               const excedente = acumulado - metaObj.objetivo;
               const extraMsg = excedente > 0 ? `<br><br>¡Y la has superado por <strong>$${excedente.toFixed(2)}</strong>!` : "";
               mostrarModalAlertaInteligente(
                 "meta_superada",
                 "¡Meta Cumplida!",
                 `¡Felicidades! Con tu abono de <strong>$${nueva.monto.toFixed(2)}</strong> has alcanzado tu objetivo para <strong>${escapeHTML(nombreMeta)}</strong>.${extraMsg}`
               );
            } else if (acumulado >= metaObj.objetivo * 0.9 && acumuladoPrevio < metaObj.objetivo * 0.9) {
               mostrarModalAlertaInteligente(
                 "advertencia",
                 "¡Casi lo logras!",
                 `Con este abono, estás a muy poco de cumplir tu meta <strong>${escapeHTML(nombreMeta)}</strong>. ¡Sigue así!`
               );
            }
          }
        }
      ...
      function mostrarModalIncremento(id, nombre, montoInicial) {
        ...
        document
          .getElementById("modalIncrementoDeuda")
          .classList.remove("hidden");
      }
      function ocultarModalIncremento() {
        document.getElementById("modalIncrementoDeuda").classList.add("hidden");
      }
      ...
      function ocultarModalMetaAhorro() {
        document.getElementById("modalMetaAhorro").classList.add("hidden");
      }
      ...
      async function cargarDatosCloud() {
        // Retornar la promesa si hay una carga en curso para evitar llamadas concurrentes
        ...
        promesaCargaInicial = (async () => {
          try {
            ...
            transacciones = fetchedTransacciones.filter(
              (t) => typeof transaccionesPendientesEliminar === "undefined" || !transaccionesPendientesEliminar.has(t.id)
            );
      ...
      document
        .getElementById("formConfigPresupuestos")
      ...
      function renderizarSeccionPresupuestos(gastosPorCat) {
        ...
        grid.innerHTML = categoriasConPresupuesto
          ...
          `;
              ...
              colorBarra =
                "bg-azulelectrico shadow-[0_0_8px_rgba(2,89,221,0.5)]";
        ...
        grid.innerHTML = metasAhorro
          .map((meta) => {
            ...
            if (!completada && acumulado > 0) {
              // Obtener depósitos promedios (simplificado: asumiendo que los depósitos se han hecho en un tiempo, o calculamos un promedio mensual).
              // Para no hacer cálculos pesados de fechas, sugerimos algo estático en base al ahorro mensual actual o global.
              // Simplificado: calculamos "si ahorras $X adicionales, lo logras".
      ...
      function seleccionarChipTipo(tipo) {
        ...
        const claseActiva =
          "chip-tipo px-3 py-1.5 rounded-xl text-xs font-bold bg-azulelectrico text-white shadow-sm transition shrink-0";
        const claseInactiva =
          "chip-tipo px-3 py-1.5 rounded-xl text-xs font-bold bg-slate-100 dark:bg-slate-950 text-slate-700 dark:text-azulcielo border border-slate-200 dark:border-azulcielo/20 hover:bg-slate-200 dark:hover:bg-slate-800 transition shrink-0";
      ...
      function limpiarFiltros() {
        ...
        document.getElementById("filtroDia").value = String(
          new Date().getDate(),
        ).padStart(2, "0");
        renderizarHistorialFiltrado();
      ...
      function renderizarHistorialFiltrado() {
        ...
        if (alcance === "dia") {
          ...
          contenedorDia.classList.add("block"); // para grid
        } else {
        ...
        const filtradas = transacciones
          ...
          .sort((a, b) => {
            // Orden descendente por fecha, y si es la misma fecha, descendente por ID (última registrada primero)
            // Usamos Math.abs() para los IDs porque los temporales offline son negativos (-Date.now())
            if (a.fecha !== b.fecha) {
        ...
        document.getElementById("conteoFiltradas").textContent =
          `${filtradas.length} de ${transacciones.length} movimiento(s)`;
        ...
        filtradas.forEach((t) => {
          ...
          if (t.tipo === "ingreso") {
            ...
            if (cat.includes("retirar") || cat.includes("usar") || cat.includes("retiro") || cat.includes("gasto")) {
              totalAhorrosHistorial -= m; // retiros disminuyen el total ahorrado en el mes
            } else {
        ...
        document.getElementById("resumenMontoFiltrado").textContent =
          `Neto: $${totalFlujo.toFixed(2)}`;
      ...
      const transaccionesPendientesEliminar = new Map();
      ...
      const editDeudaObjetivoSelect =
        document.getElementById("editDeudaObjetivo");

      function actualizarOpcionesCategoriaEdit(categoriaActual) {
        const tipo = editTipoSelect.value;
        editCategoriaSelect.innerHTML = categoriasMap[tipo]
          .map(
            (cat) =>
              `<option value="${escapeHTML(cat)}">${escapeHTML(cat)}</option>`,
          )
      ...
      editTipoSelect.addEventListener("change", () =>
        actualizarOpcionesCategoriaEdit(),
      );
        ...
        try {
          ...
          const cambios = {
            ...
            deuda_id: esPagoDeuda
              ? parseInt(editDeudaObjetivoSelect.value)
              : null,
      ...
      document
        ...
        .addEventListener("submit", confirmarEditarMovimiento);
      ...
      function actualizarInterfaz() {
        ...
        document.getElementById("subIngresos").innerHTML =
          `${countIngresos} mov.${compIngresoHTML}`;

        document.getElementById("totalGastos").textContent =
          `$${gastosMes.toFixed(2)}`;
        document.getElementById("subGastos").innerHTML =
          `${countGastos} mov.${compGastoHTML}`;

        document.getElementById("totalAhorro").textContent =
          `$${ahorroAcumuladoTotal.toFixed(2)}`;
        document.getElementById("subAhorro").textContent =
          `${Object.keys(fondosAhorroMapa).length} fondo(s)`;

        document.getElementById("totalDeudas").textContent =
          `$${totalDeudaPendiente.toFixed(2)}`;
        document.getElementById("subDeudas").textContent =
          `${deudas.length} acreedor(es)`;

        let balance = 0;
        transacciones.forEach((t) => {
          if (t.fecha.substring(0, 7) <= mesSeleccionado) {
            if (typeof transaccionesPendientesEliminar !== "undefined" && transaccionesPendientesEliminar.has(t.id)) return;
            const m = parseFloat(t.monto) || 0;
            if (t.tipo === "ingreso") balance += m;
            else if (t.tipo === "gasto") balance -= m;
          }
        ...
        const tasaAhorro =
          ingresosMes > 0
            ...
            : 0;
        document.getElementById("tasaAhorro").textContent =
          `Tasa de Ahorro: ${tasaAhorro.toFixed(0)}%`;
        ...
        metasAhorro.forEach((meta) => {
          ...
          if (meta.objetivo > 0) {
            if (acumulado >= meta.objetivo) {
              ...
              const textoExtra = excedente > 0 ? ` ¡Y la has superado por $${excedente.toFixed(2)}!` : " ¡Increíble!";
              alertasGlobalesHTML += `
                <div class="snap-center shrink-0 w-64 bg-white dark:bg-slate-900 p-4 rounded-3xl border border-emerald-500/30 shadow-md flex flex-col gap-2 relative overflow-hidden">
                  <div class="absolute -right-4 -top-4 w-16 h-16 bg-emerald-500/10 rounded-full blur-xl pointer-events-none"></div>
                  <div class="flex items-center gap-2.5 relative z-10">
